...
**Phase 09 MVP Release Closure 当前为 `blocked`。** 首个公开 MVP Release baseline 已冻结为 `0.1.0` 与 npm dist-tag `latest`，仓库内已实现 `v*` tag-trigger / existing-tag recovery、同 revision/version candidate 聚合、npm immutable artifact partial-recovery、OIDC-ready publish、public-registry exact-version verification、四平台 clean `npx` smoke 与 registry acceptance 后的 GitHub Release finalization。`@kgos` npm organization、owner authority、bootstrap secret 与 `v0.1.0` tag 已真实就绪；六个 `0.1.0` package 均已公开并可从 registry 解析，且 `latest=0.1.0`。当前剩余阻塞是完成稳定的 publish verification、四平台 post-publish registry smoke、Trusted Publisher 配置与 GitHub Release；完整状态与 Acceptance 见[Phase 09](docs/development/phases/09-mvp-release-closure.md)。
