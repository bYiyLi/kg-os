...
**KG OS 已切换到 Go 服务端/Kernel/CLI + TypeScript SDK/Web，并对齐 Lithograph v0.3.0 SQL-only integration；Phase 02 Ontology 已完成，Phase 03 Installation & Runtime Onboarding 当前为 `in_progress`。** Phase 00 工程基线、Phase 01 Runtime & Lithograph Host Foundation 与 Phase 02 Ontology 均已进入完成基线。`KG_HOME` / config / credential、SQLite Extension resolver、正式 Go/Lithograph Host、SQL query / execute / true streaming / cancellation、Semantic Provider readiness/cache mapping、explicit transaction、single-instance lock、daemon shutdown lifecycle、完整 Ontology，以及 D72 的 `doctor/install`、完整显式配置、en/zh、本机 credential fallback 与业务 Runtime auto-start 已在当前工作树实现并完成本地验收；Phase 03 仍等待最终 pushed SHA 的 Ubuntu 24.04 x64 CI，不能提前标记 `done`。
...
**Phase 03 Installation & Runtime Onboarding 当前为 `in_progress`。** 当前工作树已经实现 `kg doctor -> kg install -> kg ontology ...` 首次流程、正式 package artifact discovery/integrity、完整显式配置、中英文交互、本机 credential fallback 与并发安全的 Runtime auto-start；本地 full validation、fresh-source、真实 PTY 与真实 Lithograph 并发 auto-start 均已有证据。尚未执行 commit / push 与最终 Ubuntu 24.04 x64 CI。
