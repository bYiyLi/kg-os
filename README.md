...
**Phase 09 MVP Release Closure 当前为 `blocked`，但公开 `v0.1.0` Release 已完成。** Release Run `36219384704` 已成功完成六包 registry verification、macOS arm64/x64 + Linux arm64/x64 public-registry `npx` smoke 与 GitHub Release finalization；六个 package 均为 `0.1.0` 且 `latest=0.1.0`，`v0.1.0` 仍指向原 release revision `d35e254`。六包已配置 `bYiyLi/kg-os` / `release.yml` Trusted Publisher，并收紧为必须 2FA、禁止 bypass token；GitHub `NPM_TOKEN` secret 与 npm bootstrap token均已删除。当前唯一剩余闭环是把长期 OIDC publish workflow 的去 token 改动与状态文档提交、推送并取得远端 CI 证据，完成后才能按仓库状态规则进入 `done`。完整状态与 Acceptance 见[Phase 09](docs/development/phases/09-mvp-release-closure.md)。
