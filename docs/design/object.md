...
Ontology Patch 对 Semantic Index 的 create/delete、targets/source 变化或 source rename/type change，仍把 Schema、Binding 与必要业务数据变化放在同一个最终 State。新增或改变 Semantic definition 时由 Lithograph 做本地 Provider/config validation；失败整体 rollback，过程中不做远端 embedding 或 source backfill。向量不是 State 数据，因此不要求为每个 Knowledge element 生成内部 Property。首版单字段范围与联合检索边界见 [Ontology](ontology.md#语义索引的首版范围)；`db.index.semantic.rebuild` 只负责 committed target 的 TEMP Semantic/HNSW materialization，不承担 Provider cache维护，也不与 Patch共用 explicit transaction。
