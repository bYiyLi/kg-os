...
Embedding 由外部 Agent / Application 生成并通过普通 Property 写入；KG OS 不内置 embedding model。Ontology 中显示的 Label、Relationship Type、Property name 与 Index name 必须对应同一 State 的真实查询名称，不能使 AI 读完模型后仍无法编写正确 Cypher。
