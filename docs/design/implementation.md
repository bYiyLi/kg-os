...
- **Merge**：按 [Phase 07 Evolution Merge Session](../development/phases/07-evolution-merge.md) 实现 Merge Session 的 public conflict projection、渐进 resolution、固定 candidate revision 一致性检查与 finalize；继续复用 Phase 06 State/Object/Change 与同一个 Lithograph Host，不建立第二套 Merge workspace 或版本模型。
