...
Full-text runtime 在 extension load 后对当前 `[fulltext].analyzer` 做 connection-local FTS5 probe。新Instance的CLI省略 `--fulltext-analyzer` 时直接写入official `jieba`；Phase 10必须先按 [research](../research/jieba-tokenizer.md) 选定并pin可真实分发的tokenizer/dictionary identity，再证明四个平台Runtime最终注册同一逻辑Jieba语义。Ontology compiler 对新建或业务定义变化后必须重建的 KG OS-managed `type: fulltext` 生成当前 analyzer 与 `eventually_consistent=false`；已有 IndexDefinition 未被本次业务 Patch 触碰时保留其实际 versioned analyzer。Decoder 有意不把 analyzer 暴露到公共 Ontology。Instance init把 analyzer 标为初始化后禁止修改；实现不维护 fulltext fingerprint/generation、不比较历史配置，也不因为当前文件变化迁移已有 State。
...
2. **原样 Cypher 与值/错误映射**：公共 Graph 不附加排除内部节点的 `graphView`，完整传递 Lithograph JSON value、底层 error 与调用方显式 execution options；用户 Cypher 改变 checkout 只影响本次operation。每条write connection归还pool前必须恢复 autocommit + no active Lithograph transaction + checkout `main`，失败则discard；不能只依赖下一operation重新checkout。targeted integration必须覆盖success/error/cancel/stream early-close与多Branch并发。
...
| SQLite extension connection lifecycle | 每个实际 SQLite connection 按 official Lithograph → official Provider → caller additional config order → official Jieba加载同一resolved set，official Jieba最终registration后再probe analyzer，随后关闭任意load权限；任一connection缺official/required extension不能进入pool |
