...
- Runtime / history：每个 connection 在 extensions 加载后、进入可用池前 probe 当前 `[fulltext].analyzer`，用于保证当前 daemon 能创建/重建新的 managed Full-text Index；这个 KG OS 自己执行的 capability probe 可以使用 `FULLTEXT_ANALYZER_UNAVAILABLE`。历史 query 使用目标 State 的实际 versioned IndexDefinition analyzer；当前 connection 没有注册旧 tokenizer 时，只让该次 Lithograph Full-text 操作按其实际公开 category 失败，daemon 与普通历史 read 继续可用。Graph passthrough 不解析 Cypher/message 再猜成 KG OS analyzer code。Remote extension hash 可以固定插件 binary，但 KG OS 无法仅靠 analyzer name 检测第三方插件外部词典/资源被原地替换；这是 operator/runtime reproducibility responsibility。
