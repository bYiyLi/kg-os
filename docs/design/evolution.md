...
Merge 采用 **Merge Session**，因为冲突可能非常多，AI / 用户必须能够分页读取并跨多个调用逐步解决。KG OS 开始 merge 时先把 target Branch head 与 `source` 分别解析为 immutable `targetState/sourceState` 并完成 D31 consistency check；两端的 embedding-space fingerprint 还必须彼此相同并与当前 runtime config 匹配，否则返回 `EMBEDDING_SPACE_MISMATCH`，不能把不同向量空间作为普通 graph conflict 合并。随后调用 Lithograph `merge.start(source=sourceState, expectedHead=targetState)`，target Branch 通过 query-level `branch` 指定。Lithograph 在 Session 持久化的同一 writer boundary 内做 expected-head CAS：Branch 中间发生变化就返回 `BRANCH_HEAD_MOVED` 且**不创建 Session**；成功 Session 必须返回相同的 `targetState/sourceState`。因此无效 State 不会先变成一个可见 workspace，也不存在“先检查 ref、再由底层重新解析另一个 State”的竞态。
...
当 `unresolved=0` 时，KG OS 再次确认 Session pinned `targetState/sourceState` 仍满足 D31，然后对**该精确 revision 的 candidate**做一致性校验。校验至少覆盖 Binding coverage、reserved internal graph / Schema isolation、Ontology aggregate 可解释性与共享声明一致性、caller-owned Schema/Knowledge 不含 Vector，以及托管 semantic vector 与其 public source fields / embedding-space fingerprint 的一致性。managed vector slot 不是公共 conflict，不能让用户手工选择 ours/theirs；最终 candidate 必须按合并后的 public source 重新得到确定的向量结果后才可 finalize。若当前底层 Merge Session 只能只读检查而不能在同一 candidate revision 上安全刷新该 managed data，则 semantic-source merge 的 finalize 属于实现 readiness blocker，不能先 finalize 再补第二个隐藏 Commit。校验/刷新失败时 Session 保留且不移动 Branch。
...
`history` 返回统一 State DAG 上与调用方 scope / Object address 相关的业务变化序列；`diff` 比较两个 immutable State Snapshot。两者都把底层历史业务化为公开 Object / Knowledge graph 变化，并过滤 KG OS internal Binding Record、reserved Label / Relationship、Schema Locator、embedding-space record 与 managed vector materialization 等实现细节；semantic source Property 或 public vector Index 定义本身的变化仍按公开 Object/Knowledge 语义报告。mutable State Data、Branch 与 Tag 不进入 Snapshot diff。
...
任何会创建新 Snapshot State 或推进 Branch 到新 Snapshot 的 KG OS mutation，都要求其业务 base State 的 embedding-space fingerprint 与当前 runtime config 匹配；旧 fingerprint 的历史 State 仍可用于只读 Ontology/Knowledge/Full-text 观察，但不能在新 Provider 下继续写出混合向量空间的后代。单纯创建/移动只读引用到历史 State 不改变该历史 State 内容，但后续从该 Branch 发起写入时仍必须通过这一检查。
