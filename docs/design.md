...
| 当前实现基线 | Phase 00 Engineering Foundation、Phase 01 Runtime & Lithograph Host Foundation、Phase 02 Ontology 与 Phase 03 Installation & Runtime Onboarding 均已完成并进入 `main`；当前工作树已实现 Phase 04 General Object Read & Patch 的五类 Object batch read、Knowledge Node/Relationship 与通用 batch Patch，本地 full/fresh-source validation 和 Phase Review 已通过，阶段状态为 `in_progress`，等待最终 pushed SHA 的远端 CI。状态与完成证据由[开发计划](development/README.md)及对应 Phase 文件维护 |
...
| 工程待办 | Phase 04 当前本地实现已完成，仍需最终 pushed SHA 的远端 CI 才能 `done`；Object继续不实现 list/search。Phase 04 闭环后再实现 Graph 公共 HTTP、Evolution / Merge 及完整 SDK / Web / Skill。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
