...
| [runtime.md](design/runtime.md) | `kgosd` HTTP、OpenAI-compatible Embeddings startup config、single-instance lock、daemon lifecycle、Web hosting、`~/.kgosd/` 配置/日志/数据目录 |
| [decisions.md](design/decisions.md) | D1–D50 的决定、依据、备选和取舍，以及被替换架构记录 |
...
本次 Ontology 基线已取代逐 Schema resource 的公共投影/编辑与 Ontology search 方案；只改变上层交互和 compiler 职责，不重建 Lithograph 或第二份 Schema 真源。D48 把 embedding service 收敛为 Knowledge Base 基础配置，并把 Ontology 的 `vector` index 定义为 KG OS 托管的语义检索需求；D49 进一步冻结 KG OS v1 **不公开 caller-owned Vector Property/value/parameter/result**；D50 冻结唯一远端协议为 OpenAI-compatible Embeddings，并明确 `base_url/model/dimensions/api_key|api_key_env` 配置。batch read / scoped patch 见 D47，基线替换沿革见 D46。
