...
| 工程待办 | [Phase 08 TypeScript Client & npm Runtime Distribution](development/phases/08-typescript-client-npm-runtime.md) 已为 `done`；[Phase 09 MVP Release Closure](development/phases/09-mvp-release-closure.md) 当前为 `blocked`：`0.1.0` / `latest` baseline、`v*` tag-triggered Release tooling、`@kgos` owner authority、bootstrap secret 与 `v0.1.0` tag均已确认；首次 Release 的四平台candidate成功后在publish前aggregate gate暴露CLI build cache跨OS差异，正在通过existing-tag recovery修复，真实npm publish、registry npx验收、六包Trusted Publisher配置与GitHub Release仍待完成。Web页面/Skill继续留在后续独立阶段。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
