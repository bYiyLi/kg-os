...
| 工程待办 | [Phase 08 TypeScript Client & npm Runtime Distribution](development/phases/08-typescript-client-npm-runtime.md) 已为 `done`；[Phase 09 MVP Release Closure](development/phases/09-mvp-release-closure.md) 当前为 `blocked`：`0.1.0` / `latest` baseline、`v*` tag-triggered Release tooling 与 `@kgos` owner authority 已确认，真实首次npm bootstrap、registry npx验收、六包Trusted Publisher配置与GitHub Release等待实际Release执行。Web页面/Skill继续留在后续独立阶段。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
