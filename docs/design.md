...
| [decisions.md](design/decisions.md) | D1–D42 的决定、依据、备选和取舍，以及被替换架构记录 |
...
| 状态 | 范围与入口 |
| --- | --- |
| 已确认 | [架构](design/architecture.md)、[Ontology](design/ontology.md)、[Object](design/object.md)、[Knowledge / Graph](design/graph.md)、[Evolution](design/evolution.md)、[共享公共合同](design/contracts.md)，以及这些规则对应的 [关键设计决定](design/decisions.md) |
| 工程剩余 | [工程映射与实现待办](design/implementation.md)；只包括 Lithograph readiness、Schema / `SHOW` → `structure` projection、Object Patch → Cypher statement planning、Merge conflict projection / candidate checker，以及 `kgosd` client transport、CLI / SDK / HTTP / Skill / Web adapter mapping |
