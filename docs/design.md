...
| 当前实现基线 | Phase 00 Engineering Foundation、Phase 01 Runtime & Lithograph Host Foundation、Phase 02 Ontology 与 Phase 03 Installation & Runtime Onboarding 均已完成；D72 的 `doctor/install`、完整显式 config、CLI 本地 credential fallback、en/zh 与业务 Runtime auto-start 已进入 `main` 基线并完成本地/远端验收。状态与完成证据由[开发计划](development/README.md)及对应 Phase 文件维护 |
...
| 工程待办 | Phase 03 已完成；后续再实现普通 Knowledge Object / 通用 Object surface、Graph 公共 HTTP、Evolution / Merge 及完整 SDK / Web / Skill。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
