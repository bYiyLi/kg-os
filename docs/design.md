...
| 当前实现基线 | Phase 00 Engineering Foundation、Phase 01 Runtime & Lithograph Host Foundation、Phase 02 Ontology 与 Phase 03 Installation & Runtime Onboarding 均已完成；D72 的 `doctor/install`、完整显式 config、CLI 本地 credential fallback、en/zh 与业务 Runtime auto-start 已进入 `main` 基线并完成本地/远端验收。Phase 04 General Object Read & Patch 已完成设计与计划、状态为 `ready`，尚未实现。状态与完成证据由[开发计划](development/README.md)及对应 Phase 文件维护 |
...
| 工程待办 | Phase 04 已建立并处于 `ready`：实现五类 Object 的 batch read、Knowledge Node/Relationship 与通用 batch Patch；Object不实现 list/search。之后再实现 Graph 公共 HTTP、Evolution / Merge 及完整 SDK / Web / Skill。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
