...
| 当前实现基线 | Phase 00 Engineering Foundation、Phase 01 Runtime & Lithograph Host Foundation、Phase 02 Ontology、Phase 03 Installation & Runtime Onboarding 与 Phase 04 General Object Read & Patch 均已完成并进入 `main`；Phase 04 的五类 Object batch read、Knowledge Node/Relationship 与通用 batch Patch 已通过本地、fresh-source 与 Ubuntu CI 验收。状态与完成证据由[开发计划](development/README.md)及对应 Phase 文件维护 |
...
| 工程待办 | Phase 04 已完成；Object继续不实现 list/search。[Phase 05 Graph Query & Execute](development/phases/05-graph.md) 已进入 `ready`，下一步实现 Graph 公共 HTTP、NDJSON streaming 与 `kg graph query/execute`；Evolution / Merge 及完整 SDK / Web / Skill仍在后续。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
