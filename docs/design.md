...
| 当前实现基线 | Phase 00 Engineering Foundation、Phase 01 Runtime & Lithograph Host Foundation 与 Phase 02 Ontology 均已完成；当前工作树还已实现 D72 的 `doctor/install`、完整显式 config、CLI 本地 credential fallback、en/zh 与业务 Runtime auto-start。Phase 03 的本地实现/验收已闭环，最终 pushed SHA 的 Ubuntu 24.04 x64 CI 尚未执行，因此阶段仍为 `in_progress`；状态与完成证据由[开发计划](development/README.md)及对应 Phase 文件维护 |
...
| 工程待办 | Phase 03 当前只剩最终 pushed SHA 的 Ubuntu 24.04 x64 CI / 状态闭环；之后再实现普通 Knowledge Object / 通用 Object surface、Graph 公共 HTTP、Evolution / Merge 及完整 SDK / Web / Skill。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
