...
| 当前实现基线 | Phase 00 Engineering Foundation、Phase 01 Runtime & Lithograph Host Foundation、Phase 02 Ontology、Phase 03 Installation & Runtime Onboarding、Phase 04 General Object Read & Patch 与 Phase 05 Graph Query & Execute 均已完成并进入 `main`；Graph 公共 Kernel / HTTP / CLI、NDJSON streaming、Full-text / Semantic passthrough 与 cancellation / transport hardening 已通过主树、独立 fresh-source 与 Ubuntu CI 验收。状态与完成证据由[开发计划](development/README.md)及对应 Phase 文件维护 |
...
| 工程待办 | Phase 05 已完成；Object继续不实现 list/search。[Phase 06 Evolution Core](development/phases/06-evolution-core.md) 已进入 `ready`，下一步实现 State / State Data、Branch / Tag、History / Diff 与正式 `kg evolution` Core surface；Merge Session 及完整 SDK / Web / Skill仍在后续。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
