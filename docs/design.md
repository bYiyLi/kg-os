...
| 工程待办 | [Phase 08 TypeScript Client & npm Runtime Distribution](development/phases/08-typescript-client-npm-runtime.md) 已为 `done`；[Phase 09 MVP Release Closure](development/phases/09-mvp-release-closure.md) 当前为 `blocked`：`0.1.0` / `latest` Release baseline 与仓库内 Release tooling 已实现，真实public npm publish、registry npx验收、Git tag / GitHub Release等待npm authority与实际Release执行。Web页面/Skill继续留在后续独立阶段。具体阶段顺序见[开发计划](development/README.md)，复用边界见[工程映射](design/implementation.md) |
